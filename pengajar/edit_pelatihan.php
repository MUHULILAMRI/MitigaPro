...
$stmt = $conn->prepare("SELECT * FROM identifikasi_pelatihan WHERE id = ?");
$stmt->bind_param('i', $id);
$stmt->execute();
$data = $stmt->get_result()->fetch_assoc();
$stmt->close();

if (!$data) { header('Location: ' . BASE_URL . 'pengajar/dashboard.php'); exit; }

if (isset($_POST['simpan'])) {
    if (!csrf_verify()) { header('Location: ' . $_SERVER['REQUEST_URI']); exit; }
    $jenis = trim($_POST['jenis_pelatihan'] ?? '');

    $stmt = $conn->prepare("UPDATE identifikasi_pelatihan SET jenis_pelatihan = ? WHERE id = ?");
    $stmt->bind_param('si', $jenis, $id);
    $stmt->execute();
    $stmt->close();

    header("Location: detail_dinas.php?id=" . $data['dinas_id']);
    exit;
}
?>
...
<title>Edit Pelatihan | MitigaPro</title>
...
.form-card{background:var(--white);border-radius:var(--radius);border:1px solid var(--border);width:100%;max-width:480px;overflow:hidden;margin:0 auto}
...
<?= breadcrumb([['label' => 'Data Dinas', 'url' => 'dinas.php'], ['label' => 'Edit Pelatihan']]) ?>
<div class="form-card">
  <div class="form-header">
    <h2><i class="fas fa-edit"></i> Edit Pelatihan</h2>
    <p>Perbarui data pelatihan</p>
  </div>
  <div class="form-body">
    <form method="POST">
      <?= csrf_field() ?>
      <div class="fg">
        <label><i class="fas fa-graduation-cap"></i> Nama Pelatihan</label>
        <input type="text" name="jenis_pelatihan" value="<?= htmlspecialchars($data['jenis_pelatihan']) ?>" required>
      </div>
      <div class="form-actions">
        <button type="submit" name="simpan" class="btn btn-primary"><i class="fas fa-save"></i> Simpan</button>
        <a href="detail_dinas.php?id=<?= (int)$data['dinas_id'] ?>" class="btn btn-secondary"><i class="fas fa-arrow-left"></i> Kembali</a>
      </div>
    </form>
  </div>
</div>
