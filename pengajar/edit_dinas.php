...
if (isset($_POST['simpan'])) {
    ...
    $stmt = $conn->prepare("UPDATE dinas SET nama_dinas = ? WHERE id = ?");
    $stmt->bind_param('si', $nama_dinas, $id);
    $stmt->execute();
    $stmt->close();

    header("Location: wilayah.php?id=" . $data['wilayah_id']);
    exit;
}
?>
...
<title>Edit Dinas | MitigaPro</title>
...
<?= breadcrumb([['label' => 'Data Dinas', 'url' => 'dinas.php'], ['label' => 'Edit Dinas']]) ?>
<div class="form-card">
  <div class="form-header">
    <h2><i class="fas fa-edit"></i> Edit Dinas</h2>
    <p>Perbarui nama dinas</p>
  </div>
  <div class="form-body">
    <form method="POST">
      <?= csrf_field() ?>
      <div class="fg">
        ...
        <input type="text" name="nama_dinas" value="<?= htmlspecialchars($data['nama_dinas']) ?>" required>
      </div>
